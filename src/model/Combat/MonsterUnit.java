...
//
//    //TODO : il n'y pas de grange dans la grotte donc il faudra soit supprimer cette attribut soit le changer pour une autre zone d'obstacle à éviter
//    private static final int HITBOX_SIZE = 20;
//    // Rayon de détection des du joueur pour l'attaquer
//    private static final int PLAYER_DETECTION_RADIUS = 120;
//    // Le délai de 5s avant de manger la culture une fois que l'on est positionné dessus.
//    // techniquement pas de délai pour attaquer le joueur donc faudrait le supprimer ca
...
//    // si le monstre ne trouve pas le joueur
...
//    };
//
//    // Position courante du monstre sur l'axe horizontal.
//    private volatile double x;
//    // Position courante du monstre sur l'axe vertical.
//    private volatile double y;
//    // Cible courante vers laquelle le monstre se dirige sur l'axe horizontal et vertical. cad le point ou se trouve le jouer
...
//    // Indique si le monstre repart vers le terrier après une recherche infructueuse.
//    private volatile boolean isReturningToBurrow = false;
//    // Indique si le monstre a complètement quitté l'écran après une sortie de carte.
...
//    // Compteur du nombre d'updates pendant lesquels le monstre a presque cessé de bouger.
...
//    // pas de grange donc pas utile mais à renomer pour autre chose
...
//
//    // Gestionnaire d'objectifs pour mettre à jour les objectifs liée aux kills.
...
//     */
//    public synchronized void update(MonsterModel monsterModel, Unit player, GrilleGrotte grilleGrotte, int viewportWidth, int viewportHeight,
...
//        handleFleeTrigger(monsterModel, player);
//        updateCultureTarget(monsterModel, grilleGrotte);
//        updateBurrowReturnState(monsterModel);
//        ensureValidTarget(player);
//        updateNavigationState();
//        moveTowardTarget();
//        updateCultureWaiting(monsterModel, grilleGrotte);
...
//
//    /**
//     * Déclenche la fuite si le monstre entre dans la zone d'influence du joueur.
//     * Cette méthode a priorité sur le reste : un monstre en fuite abandonne immédiatement sa culture et son déplacement normal.
//     * // TODO : le monstre ne fui pas techniquement, donc on peut supprimer cette méthde
//
...
//     * Tant que le monstre n'est pas occupé ou à sortir, on le fait entrer dans le champ
...
//
//
//        // TODO : à changer car il n'y a pas de culture comme target mais un player
//        if (isLeavingMap()) {
//            if (hasPlayerTarget()) {
//                return;
//            }
...
//            return;
//        }
//        // TODO : isInsideMap pour GrilleGrotte ?
//        if (isInsideMap) {
//            pickNewTarget();
//        }
//    }
...
//
//        // TODO : pas de culture donc à changer pour le player
...
//
//    /**
//     * Marque définitivement le Monstre comme sorti quand il a réellement quitté l'écran.
...
//
//    /**
//     * Cherche la présence du joueur
//     * TODO : le target se fait non pas sur une case de la grotte mais sur le point du joueu donc on ne changerait pas grilleGrotte par player ? .
//     */
//    // TODO : reprendre la logique de détection du joueur pour que le monstre puisse le poursuivre et pas  fuir
//    private void updatePlayerTarget(MonsterModel monsterModel, Unit player) {
//        if (grilleGrotte == null || isFleeing) {
//            clearCultureTarget(monsterModel);
//            return;
//        }
//
//        if (hasCultureTarget()) {
...
//     */
//    private void updateBurrowReturnState(MonsterModel monsterModel) {
//        if (isFleeing || isReturningToBurrow) {
//            // TODO : NOPLAYERFONDSTARTIME
//            noCultureFoundStartTime = -1L;
//            return;
//        }
//
...
//        }
//
//        startBurrowReturn(monsterModel);
//    }
//
//
//
...
//
//    /**
//     * Sert de test rapide pour savoir si l'IA a déjà LE JOUEUR réservée en ligne de mire.
//
...
//     * // TODO : utile pour anuler la cible du player ?
//
//    private void clearCultureTarget(MonsterModel monsterModel) {
...
//        if (monsterModel != null && hasCultureTarget()) {
//            monsterModel.releaseCultureReservation(targetCultureGridX, targetCultureGridY, this);
//        }
...
//     * TODO : PAS DE FUITES DONC PAS UTILE, mais à revoir pour faire en sorte que le monstre puisse poursuivre le joueur
//     */
...
//     */
//    private void startBurrowReturn(MonsterModel monsterModel) {
//        isReturningToBurrow = true;
//        // TODO : pas de culture donc à supprimer ou à changer pour le player
//        clearCultureTarget(enemyModel);
//        updateBurrowReturnTarget();
//    }
//
...
//
//    /**
//     * Donne au monstre une petite destination aléatoire de promenade.
...
//
//    /**
//     * Applique un déplacement seulement s'il laisse le mon dans une zone autorisée.
...
//        // TODO : modifier preferredTurnSign pour qu'il soit plus pertinent dans le contexte du player que de la grange
...
//     * Pendant un retour au terrier, la présence du joueur peut faire que le lapin hésite entre rester ou repartir, ce qui n'est pas vraiment une fuite effective.
//     */
//    private boolean isActivelyLeavingMap() {
//        // TODO : TOUJOURS PAS DE CULTURE TARGET DONC À CHANGER POUR LE PLAYER
...
//
//    /**
//     * Indique au modèle si ce mostre peut être retiré de la liste active.
//     */
...
//     * // TODO : Attaque pas de culture mais de player donc à revoir pour que ça indique si le monstre est en train de poursuivre le player ou pas, et que le chrono corresponde à ça
//     */
